...
price_limits[1] <- max(c(price_limits[1], 2))
...
As expected - the more reviews, the more positive reviews.
...
product_data %>% 
  ...
  filter(avg_stars %in% as.character(2:5)) %>% 
...
It seems the number of reviews per product is not really dependent on the price.

```{r n_rev_price}
product_data %>% 
  ggplot(aes(x = price, y = n_rev_per_asin)) +
  geom_smooth(method = "loess", se = TRUE) +
  scale_x_continuous(limits = price_limits) +
  expand_limits(y = 1) +
